  ...
  let comment=$("#comment").val();
   ...
   success:function(response)
   {
       if(response)
      {
           ...
           $("#commenttable tbody").prepend('<tr id="user-'+response.id+'"><td>'+response.id+'</td><td>'+response.is_replay_to_id+'</td><td>'+response.comment+'</td><td>'+response.enable+'</td><td>'+response.user_id+'</td><td><button onclick="editrecord('+response.id+')"><i class="material-icons">mode_edit</i></button><button onclick="deleterecord('+response.id+')"><i class="material-icons">delete_sweep</i></button></td></tr>');
            $("#commentform")[0].reset();
